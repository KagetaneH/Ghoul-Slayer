...
public class EnemySystem extends EntitySystem implements EntityListener
{
    ...
    {
        GameState.__ENEMY_SPEED__ = GameState.__ENEMY_STARTING_SPEED__;
    ...
    {
        if(entities.size() < GameState.__ENEMY_SPAWN_NUMBER__)
        {
        ...
        for(Entity e : entities)
        {
            ...
            {
                GameState.__ENEMY_SPEED__ = GameState.__ENEMY_STARTING_SPEED__ + (float) PlayerComponent.__KILL_COUNTER__ / 400;
                cm.get(e).walkDirection.scl(GameState.__ENEMY_SPEED__);
            }
            ...
            else if(e.getComponent(EnemyComponent.class).state == EnemyComponent.STATE.HURT)
            {
                ...
                if(e.getComponent(StatusComponent.class).checkHurtTime())
                {
                    ...
                    if(GameState.__ENEMY_SPEED__ >= GameState.__ENEMY_RUNNING_SPEED__)
                    {
                        enemyAnimationComponent.animate(EnemyAnimations.__RUN_ANIMATION__, -1, 0.5f);
                    }
                    else
                    {
                        enemyAnimationComponent.animate(EnemyAnimations.__WALK_ANIMATION__[(int) (Math.random() * 10) % 2], -1, 0.5f);
                    }
                }
            }
